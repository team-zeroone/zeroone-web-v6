...
CRITICAL: Your response must start with exactly "EXCERPT:" on the first line. Any deviation from the output format will break the application.

RULES:
1. Tone: "This platform empowers commuters by reducing friction..." (innovative, confident, sleek). Not salesy or generic.
2. Body length: 60-100 words total across all sections. Focus strictly on high-level business value, problem solved, and design philosophy. Do NOT include detailed technical/architecture breakdowns, as that will be shown in a separate diagram.
...
5. NEVER use the "—" character (em-dash, U+2014). Use "-", ":", or "," instead.
6. No preamble, no explanation, no frontmatter.
7. Body format: Use 2-3 short, punchy sections. Each section has a ### subheading and 1-2 sentences max beneath it. No long paragraphs.

You MUST follow this exact output format:
Line 1: EXCERPT: [your refined 15-25 word summary]
Line 2: blank
Line 3 onwards: the Markdown body, using short sections as described in Rule 7.
...
### The Problem
Urban commuters faced slow, unreliable ride access - a fragmented experience with no real-time visibility.

### The Solution
A cross-platform mobile app delivering sub-second ride matching through a custom geolocation engine, built for speed and simplicity.

### The Impact
50k+ active users onboarded, with a frictionless interface that redefined the standard for modern urban mobility.`;
...
Rules:
1. Return ONLY the raw Mermaid code block. No explanations, no preamble, no commentary — not a single word before or after the code block.
...
7. Node IDs must be strictly alphanumeric with no spaces or special characters.
   - BAD:  api auth,  my-node,  node_1
   - GOOD: apiAuth,   myNode,   node1
